...
%sq_50
mag_sq50_envresponse = fft(sq50_envresponse);
T = 1/Fs; %Sampling Period
...
%sq_25
mag_sq25_envresponse = fft(sq25_envresponse);
T = 1/Fs; %Sampling Period
...
mag_SAM_envresponse = fft(SAM_trunc);
T = 1/Fs; %Sampling Period
L = length(SAM_trunc);
P2 = abs(mag_SAM_envresponse/L);
P1_SAM = P2(1:L/2+1); 
...
mag_sq50_envresponse = fft(sq50_trunc);
T = 1/Fs; %Sampling Period
L = length(sq50_trunc);
P2 = abs(mag_sq50_envresponse/L);
P1_sq50 = P2(1:L/2+1); 
...
mag_sq25_envresponse = fft(sq25_trunc);
T = 1/Fs; %Sampling Period
L = length(sq25_trunc);
P2 = abs(mag_sq25_envresponse/L);
P1_sq25 = P2(1:L/2+1); 
