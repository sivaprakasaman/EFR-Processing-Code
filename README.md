# Investigating the Diagnostic Potential of Differing Auditory Stimuli in Synaptopathy- EFR Processing Code
...
![Figure 2](Figures/noisefloorremoval.png)
***Figure 2.*** *Example of noise floor removal.*

Simply subtract the noise floor from the mean of MRS to get the SNR.
...
`getSum` is a method used in `EFR_Processing.m` for basic analysis based on the sum of N harmonic values (either SNR or PLV). It is responsible for calculating the data represented in the bottom two graphs of [Figure 1](Figures/PLV_MAG_Both.JPG).
...
## Plotting means and standard devs as a function of trials collected using `Plotter.m`

`Plotter.m` was made in order to help justify the number of trials we collected per subject. The ideal number of trials is the lowest number where the harmonic sums of the 3 stimuli stop overlapping:
...
**Andrew Sivaprakasam** - Purdue University and Indiana University School of Medicine, Graduate Student & Medical Student
